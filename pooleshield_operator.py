#!/usr/bin/env python3
"""
PooleShield v5.1.0 operator CLI.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="PooleShield v5.1 real operator workflow, desktop prototype, and portable Windows build helper")
    ...
    rule_pack_load_cmd = sub.add_parser("rule-pack-load", help="Load metadata-only rule pack entries for the v5.1 Rule Pack Editor UI")
    ...
    results_load_cmd = sub.add_parser("results-load", help="Load metadata-only scan results for the v5.1 Results UI")
    ...
    baseline_load_cmd = sub.add_parser("baseline-load", help="Load metadata-only trusted baseline entries for the v5.1 Baseline Manager UI")
    ...
    desktop_cmd = sub.add_parser("desktop", help="Launch the v5.1 local desktop UI prototype")
    desktop_cmd.add_argument("--status", action="store_true", help="Print desktop dependency status instead of launching UI")


    portable_cmd = sub.add_parser("portable-build", help="Plan or run the v5.1 portable Windows PyInstaller build")
    ...
    installer_cmd = sub.add_parser("installer-build", help="Plan or run the v5.1 Windows installer build")
    ...
    if args.command == "installer-build":
        ...
        try:
            if args.status:
                ...
                summary = run_iscc(args.script_path, root=".")
            else:
