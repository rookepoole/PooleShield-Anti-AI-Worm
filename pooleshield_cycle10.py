#!/usr/bin/env python3
"""
PooleShield Cycle 10 one-command runner.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="PooleShield v1.8 Cycle 10 clean-output review-ledger runner")
    parser.add_argument("--path", "-p", nargs="+", default=["examples/corpus_scan_fixture"], help="File/folder path(s) to scan")
    parser.add_argument("--output-dir", default="out/cycle10", help="Folder for generated reports. Defaults to out/cycle10")
    parser.add_argument("--clean-output", action="store_true", help="Delete --output-dir before writing new reports")
    parser.add_argument("--normalized", default="cycle10_normalized_events.jsonl", help="Normalized event output")
    parser.add_argument("--scan-output", default="cycle10_scan_report.json", help="JSON scan report")
    parser.add_argument("--scan-csv", default="cycle10_scan_report.csv", help="CSV scan report")
    parser.add_argument("--manifest", default="cycle10_quarantine_manifest.json", help="JSON quarantine manifest")
    parser.add_argument("--manifest-md", default="cycle10_quarantine_manifest.md", help="Markdown quarantine manifest")
    ...
    parser.add_argument("--policy-output", default="cycle10_policy_decisions.json", help="JSON policy-decision report")
    parser.add_argument("--policy-csv", default="cycle10_policy_decisions.csv", help="CSV policy-decision report")
    parser.add_argument("--policy-md", default="cycle10_policy_decisions.md", help="Markdown policy-decision report")
    parser.add_argument("--queue-output", default="cycle10_approval_queue.json", help="JSON approval queue")
    parser.add_argument("--queue-csv", default="cycle10_approval_queue.csv", help="CSV approval queue")
    parser.add_argument("--queue-md", default="cycle10_approval_queue.md", help="Markdown approval queue")
    parser.add_argument("--template-csv", default="cycle10_review_ledger_template.csv", help="Editable review-ledger CSV template")
    parser.add_argument("--template-json", default="cycle10_review_ledger_template.json", help="Review-ledger template JSON")
    parser.add_argument("--template-md", default="cycle10_review_ledger_template.md", help="Review-ledger template Markdown")
    ...
    parser.add_argument("--demo-ledger-csv", default="cycle10_review_decisions_demo.csv", help="Demo review decisions CSV")
    parser.add_argument("--effective-output", default="cycle10_effective_policy_decisions.json", help="Effective decisions JSON after ledger application")
    parser.add_argument("--effective-csv", default="cycle10_effective_policy_decisions.csv", help="Effective decisions CSV")
    parser.add_argument("--effective-md", default="cycle10_effective_policy_decisions.md", help="Effective decisions Markdown")
    parser.add_argument("--allowlist", default="cycle10_allowlist.json", help="Review-generated allowlist JSON")
    parser.add_argument("--denylist", default="cycle10_denylist.json", help="Review-generated denylist JSON")
